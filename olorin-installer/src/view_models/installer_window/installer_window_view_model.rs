...
use olorin_engine::app_provisioner::installer::app_installer::AppInstaller;
use olorin_engine::app_provisioner::installer::install_phase::InstallPhase;
use olorin_engine::app_provisioner::operations::launch::update_operation_launch::UpdateOperationLaunch;
use olorin_engine::app_provisioner::{app_provisioner_config::AppProvisionerConfig, progress_tracker::ProgressTracker};
...
impl InstallerWindowViewModel {
    ...
    fn on_launch_app() {
        match AppProvisionerConfig::get_default_install_dir() {
            Ok(app_dir) => {
                let app_path = app_dir.join("olorin.exe");
