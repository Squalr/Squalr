...
- need human verification: Memory viewer now has a right-side interpretation panel with compact `+` add buttons per interpreted type, truncated array/hex previews, aligned numeric array interpretations, exact-size scalar interpretation gating, and `Ctrl+A` full-page selection; add-to-project still defaults to `u8` for one byte or `u8[n]` for multi-byte selections unless an interpretation row overrides the type.
...
- Memory viewer interpretation actions are now compact `+` buttons instead of repeating the type name in the button label; the row label/tooltip carries the type context while keeping the right edge visually lighter.
