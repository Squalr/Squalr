...
8. Migrate Symbol Tree to Details.
   ...
   - Done: moved Symbol Tree list row rendering, inline rename row handling, context menu composition, plugin menu discovery, and row action collection into `squalr/src/views/symbol_tree/symbol_tree_list_view.rs`. The parent now applies typed list actions for cross-window focus and command dispatch.
   - Done: moved Symbol Tree runtime data synchronization into `squalr/src/views/symbol_tree/symbol_tree_runtime_data_controller.rs`, including scalar virtual snapshot reads, pointer target refreshes, preview value reads, symbol details memory reads, and runtime tree construction.
   - Done: moved Symbol Tree command dispatch and cross-window effects into `squalr/src/views/symbol_tree/symbol_tree_command_dispatcher.rs`, including memory/code focus, layout editor opening, rename/delete/create commands, add-to-project, plugin action execution, and delete confirmation requests.
...
- Current GUI inline-test cleanup pass removed temporary/scaffolding inline tests from `squalr/src/views` and `squalr/src/ui`, including child widget/view-data modules. `rg "#\[cfg\(test\)\]|mod tests" squalr/src/views squalr/src/ui` now has no matches. Remaining focused Symbol Layout cleanup is constants ownership and root `Widget::ui` shortcut/routing shrinkage. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Symbol Tree list extraction pass moved row/list rendering, inline rename storage handling, context menu labels/items, plugin action menu item discovery, row size formatting, and row-driven action collection into `symbol_tree_list_view.rs`. `SymbolTreeView` now keeps command/window effects for those actions while no longer owning list/context-menu rendering. `symbol_tree_view.rs` is now about 1,897 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_tree --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Symbol Tree runtime-data extraction pass moved virtual snapshot IDs/refresh intervals, scalar value collection, pointer-chain snapshot planning, preview snapshot planning/formatting, symbol details memory reads, and two-pass runtime tree construction into `symbol_tree_runtime_data_controller.rs`. `SymbolTreeView` now asks the controller for `SymbolTreeRuntimeData` and for Details projections. `symbol_tree_view.rs` is now about 1,094 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_tree --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Symbol Tree command-dispatch extraction pass moved cross-window focus and project-symbol/project-item command effects into `symbol_tree_command_dispatcher.rs`. `SymbolTreeView` now delegates list actions, takeover confirmation commands, create-module commands, define-field creation, add-to-project, rename, delete, layout-editor opening, and plugin action execution to the dispatcher. `symbol_tree_view.rs` is now about 775 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_tree --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Symbol Tree details/takeover extraction pass moved Struct Viewer Details focus/edit routing into `symbol_tree_details_focus.rs`, routed the list's focus-key comparison through that owner, and moved delete/define-field/create-module takeover panel composition into `symbol_tree_takeover_host_view.rs`. `SymbolTreeView` now keeps orchestration, shortcut routing, list action dispatch, and command handoff; it is about 517 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_tree --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Project Hierarchy de-monolith pass moved Details focus/edit/write routing into `project_hierarchy_details_focus.rs`, shared rename request construction through `project_item_rename_request_builder.rs`, moved value-edit/delete/promote-conflict takeover panels into `project_hierarchy_takeover_host_view.rs`, and moved runtime preview interval/virtual snapshot sync into `project_hierarchy_runtime_preview_controller.rs`. `project_hierarchy_view.rs` is now about 1,791 lines; the next heavy target is the row list/context-menu/drag-drop body. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr project_hierarchy --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Project Hierarchy list/context extraction pass moved row traversal, row rendering handoff, inline rename storage, icon resolution, and drag/drop indicator handling into `project_hierarchy_list_view.rs`; moved project-item context menu planning/rendering and pointer-scanner context actions into `project_hierarchy_project_item_context_menu_view.rs`; and moved reusable create-item menu rows into `project_hierarchy_create_item_menu_view.rs`. `project_hierarchy_view.rs` is now about 1,149 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr project_hierarchy --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
- Current Symbol Resolver de-monolith pass moved the resolver catalog list/header/context menu into `symbol_resolver_list_view.rs`, recursive resolver-node tree rendering into `symbol_resolver_node_tree_view.rs`, create/rename/open/delete takeover panels into `symbol_resolver_takeover_host_view.rs`, Struct Viewer details focus/edit routing into `symbol_resolver_details_focus.rs`, and resolver upsert/delete command effects into `symbol_resolver_command_dispatcher.rs`. `symbol_resolver_editor_view.rs` is now about 456 lines and keeps orchestration, toolbar, shortcut routing, and save/cancel handoff. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_resolver_editor --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
