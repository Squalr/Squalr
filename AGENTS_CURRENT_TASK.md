...
- need human verification: Memory viewer now has a right-side interpretation panel with per-type inline add buttons, truncated array/hex previews, aligned numeric array interpretations, and `Ctrl+A` full-page selection; add-to-project still defaults to `u8` for one byte or `u8[n]` for multi-byte selections unless an interpretation row overrides the type.
