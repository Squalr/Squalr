...
- Extend the layout mutation service beyond this first pass. It still needs explicit resize operations, delete-to-filler/merge policy, structured warning payloads, and a clearer distinction between shrink/shift and replace-with-filler delete semantics.
...
- Add remaining focused tests for splitting tail spans, selection stability after split, delete-to-filler merge behavior, resize warnings, pointer-to-struct sizing, and struct-layout-backed module fields. Repeated `u8[]` split and overlap rejection now have engine tests.
...
- Module-space create/update/delete has started moving behind `ProjectSymbolLayoutMutation`, but this is not yet a complete struct-layout editing service. The next hard part is resize/delete policy and warning payloads that the GUI can show before applying changes.
