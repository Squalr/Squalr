...
- Current Symbol Layout inline-test cleanup pass removed temporary/scaffolding tests from `symbol_layout_editor_view.rs` and test-only helpers from child view modules (`details/symbol_layout_details_focus.rs`, `authoring/symbol_layout_variant_session.rs`). No inline tests remain under `symbol_layout_editor_view/`; parent `symbol_layout_editor_view.rs` is now 392 lines. Remaining focused cleanup is constants ownership and root `Widget::ui` shortcut/routing shrinkage. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_layout_editor --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
