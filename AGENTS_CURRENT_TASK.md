...
- needs human verification: Reverted the mistaken unified target/item direction. `ProjectItem` is again a neutral metadata container with no target payload; the concrete built-in item created by Project Explorer/create callers is `address`; `ProjectItemsCreateRequest` carries address/module fields; pointer scanner and symbol explorer "Add to Project" now create address items from resolved addresses. The old `ProjectItemTarget` module, generic `item` type, target-kind details selector, and explicit plugin target kind were removed.
...
- Desired model: `ProjectItem` stores common item metadata only. Concrete item behavior belongs to registered project item types such as address now and script later.
