...
- needs human verification: The create/add path is now target-driven. `ProjectItemsCreateRequest` carries `ProjectItemTarget` instead of the old address/pointer/module/symbol-ref request fields; memory viewer, code viewer, pointer scanner, Symbol Tree "Add to Project", and TUI memory creation all send unified targets; scan-result add has regression coverage proving added items persist `ProjectItemTarget::Address`; and Project Explorer now offers `New Project Item` plus `New Folder` instead of old New Address/New Pointer/New Symbol Ref options. Reverified with `cargo fmt --all`, `cargo test -p squalr-engine-api project_item -- --nocapture`, `cargo test -p squalr-engine project_items -- --nocapture`, `cargo test -p squalr symbol_explorer -- --nocapture`, `cargo test -p squalr memory_viewer -- --nocapture`, `cargo test -p squalr pointer_scanner -- --nocapture`, `cargo test -p squalr code_viewer -- --nocapture`, `cargo test -p squalr project_hierarchy -- --nocapture`, `cargo test -p squalr struct_viewer -- --nocapture`, `cargo test -p squalr-tui memory_viewer -- --nocapture`, `cargo test -p squalr-tests project_items_create -- --nocapture`, `cargo check -p squalr`, `cargo check -p squalr-cli`, and `cargo check -p squalr-tui`.
- needs human verification: Project Explorer details now exposes a target-kind selector for normal items and target-specific editable fields for address, pointer, symbol, and a minimal plugin target shape. `None` is no longer a visible target kind; generic project items default to an Address target; and changing target kind refocuses/rebuilds Details immediately so the newly relevant fields appear without a manual reselection. Target edits persist through the existing details edit path and project item refresh now compares target content. Plugin payload display is read-only until the target plugin/editor registration model exists.
- Refactor Project Explorer project items into one user-facing item concept with a unified `ProjectItemTarget`. Current slices in place: item storage/resolution prefers targets, create/add callers send targets, Project Explorer can create a generic `item`, and details can switch/edit the stored target.
...
- Continue replacing old address, pointer, and symbol-ref UI assumptions with target-driven behavior. Preview, activation/freeze, value editing, detail display, and list refresh mostly resolve through `ProjectItemTarget`, but legacy item types still exist internally while the unified `item` type rolls out.
