...
- Added `SymbolicResolverDefinition` / `SymbolicResolverDescriptor`. Resolver nodes are currently literal, local field, type size, and binary arithmetic operation. Binary operations are intentionally fixed to two children for simpler editing and operator swapping.
...
- The resolver editing flow is a Project Explorer-style tree plus header toolbar: resolver rows are top-level entries, operation rows parent two child node rows, and the toolbar has a single add button that opens a Project Explorer-style add menu. Tree rows do not draw type icons. There is no search box, group box, or in-window details strip.
- Resolver and resolver-node edits now route through the existing Details Viewer (`StructViewerViewData`) by focusing resolver-specific virtual fields. Node kind, operator, and type-size data type use Details Viewer presentations rather than custom controls inside the Symbol Resolvers window. Changing node kind refocuses Details Viewer because the selected node's editable fields change shape. Needs human verification for live GUI ergonomics.
