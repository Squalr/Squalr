...
- Run rooted-device validation once on a host with Android SDK/NDK env vars + connected rooted device, then capture successful smoke output (install + launch + privileged worker check). Progress (2026-02-22): `python ./squalr-android/build_and_deploy.py --compile-check` now succeeds fully; `python ./squalr-android/build_and_deploy.py --debug` now builds + installs APK and pushes worker on a connected device, but fails at privileged step with `/system/bin/sh: su: inaccessible or not found` when running `adb shell su -c 'chmod +x /data/local/tmp/squalr-cli'`. Remaining blocker is rooted-device availability (or root shell access) rather than host toolchain configuration. Follow-up (2026-02-22): launch scripts now start explicit component `com.squalr.android/android_activity.MainActivity` first and only fall back to resolved launcher activity when needed; added compatibility entrypoint `python ./squalr-android/run-apk.py` that forwards to `run_apk.py`.
    - Owner: Just tried doing `python ./squalr-android/run-apk.py after the install, still boots to the default android screen, default android app icon, etc. (Note: I interrupted your session, you have unstaged files from previous iteration)
...
- Launcher hardening (2026-02-22): `run_apk.py` and `build_and_deploy.py` now launch explicit `com.squalr.android/android_activity.MainActivity` before any fallback activity resolution, and reject resolver outputs that do not belong to the requested package.
