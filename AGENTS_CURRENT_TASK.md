...
- Current Symbol Resolver de-monolith pass moved the resolver catalog list/header/context menu into `symbol_resolver_list_view.rs`, moved recursive resolver-node tree rendering into `symbol_resolver_node_tree_view.rs`, and moved create/rename/open/delete takeover panels into `symbol_resolver_takeover_host_view.rs`. `symbol_resolver_editor_view.rs` is now about 1,063 lines. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_resolver_editor --lib --locked`, and `git diff --check`. Needs human verification in the GUI.
