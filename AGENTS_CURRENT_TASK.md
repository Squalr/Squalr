...
- The resolver editing flow is a Project Explorer-style tree plus header toolbar/details strip: resolver rows are top-level entries, operation rows parent two child node rows, the toolbar has a single add button that opens a Project Explorer-style add menu, and node type changes live in the details strip combo box. Tree rows do not draw type icons. There is no search box or group box in the edit surface.
