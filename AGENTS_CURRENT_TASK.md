...
- The pointer-scanner toolbar now uses themed `DataValueBox` inputs for target / validation / depth / radius, a restricted `DataTypeSelectorView` for `u32` / `u64`, and themed action buttons. The results grid now uses column separators and mono text for address-heavy fields.
