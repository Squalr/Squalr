...
- Linux debugger implementation status:
  ...
  - Follow-up fix: checked code writes now pause an active debugger session for the opened process before writing code bytes and resume it afterward if it was previously running, matching the synchronization used by patch apply/no-operation/restore commands. This is intended to avoid Code Viewer instruction edits racing Linux ptrace watchpoint handling and needs human verification in the live GUI.
  - Follow-up feature: Code Viewer right-click now supports Copy bytes, Copy instruction, Copy address, Paste bytes, and Paste instruction. Paste uses the current instruction or selected instruction range as its size baseline; exact-size pastes write immediately, shorter pastes prompt to fill remaining bytes with no-operations, and longer pastes warn before overwriting following instruction bytes. This needs human verification in the live GUI.
  - Follow-up fix: x86/x64 instruction parsing now ignores disassembler branch-size hints such as `short`/`near` on branch targets, so copied text like `jge short 01003007h` can be pasted and assembled. Code Viewer oversized paste warnings now expand to instruction boundaries: clean boundary hits prompt `Overwrite Multiple`, while partial-instruction overlaps prompt `Fill + Overwrite` and append no-operations to the boundary on acceptance. This needs human verification in the live GUI.
  - Follow-up polish: Code Viewer instruction write warnings now use a two-row layout with clipped message text above stable action buttons below. Warning prompts always include Cancel, and action buttons are clamped into view even when the assembly/text column is narrow. This needs human verification in the live GUI.
  - Follow-up polish: Code Viewer warning Cancel buttons now use the theme secondary control fill and secondary-dark border instead of a transparent fill, so the button remains visible on warning backgrounds. This needs human verification in the live GUI.
  - Investigation note: a raw WSL `/proc/<pid>/mem` patch from `mov [404090h], eax` to `mov [404090h], ebx` at `0x401563` did not hang the disposable helper; the helper stayed alive and wrote the stale `ebx` value instead of the prompted `eax` value. If the GUI still appears hung after this fix, suspect debugger/session state or expected value confusion rather than the EBX instruction encoding itself.
...
- Linux WSL sanity check:
  ...
  - After adding Code Viewer copy/paste actions, `cargo build -p squalr --locked` completed successfully in WSL again.
  - After fixing branch-hint paste assembly and boundary-aware paste warnings, `cargo build -p squalr --locked` completed successfully in WSL again.
  ...
  - After switching the Code Viewer warning Cancel button to themed secondary colors, `cargo build -p squalr --locked` completed successfully in WSL again.
