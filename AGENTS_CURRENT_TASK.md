...
- Symbol Struct Editor field layout formulas moved out of the dense field rows into a per-field takeover panel opened from the properties button. The panel exposes shape, offset mode, field/type expression pickers, operator buttons, text escape hatches, and a descriptor preview. Needs human verification for live GUI ergonomics.
- Struct layout draft validation now rejects local expression dependency cycles between named fields before saving. The runtime symbolic struct resolver now uses bounded convergence passes, allowing explicitly-offset scalar fields later in a layout to feed earlier formulaic fields without recursive evaluation.
- Validation: `cargo test -p squalr-engine-domain --lib`, `cargo test -p squalr --lib symbol_struct_editor`, and `cargo test -p squalr --lib` passed on 2026-05-04.
