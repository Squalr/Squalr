...
- Need human verification: the GUI element scanner data type dropdown now stays visible but disabled after the first scan starts and remains non-interactive until `New scan` resets the scanner state; the combo label/icon render with preview coloring and the popup cannot be reopened mid-scan.
...
- Follow-up for the element scanner toolbar: `squalr/src/ui/widgets/controls/combo_box/combo_box_view.rs` now supports a disabled visual/input state, and `squalr/src/views/element_scanner/scanner/element_scanner_toolbar_view.rs` uses `ElementScannerViewState::has_active_scan()` to disable the data type dropdown whenever scan state is not `NoResults`. `cargo test -p squalr --locked` passed.
