...
- Promote-to-symbol no longer replaces address/pointer project items with `symbol_ref` items. It updates the symbol catalog and keeps the source project item, synchronizing its data type to the promoted/reused symbol. Existing `symbol_ref` conversion/delete compatibility paths remain for legacy items, but promotion no longer creates new ones. This needs human verification in the running UI.
...
- Project Explorer Details pointer-size selector now shows the data type icon for concrete pointer sizes in the closed combo and dropdown rows while keeping `None` iconless. This needs human verification in the running UI.
...
- Pointer chains now store offsets as typed `PointerChainSegment` values, serialized as JSON numbers for raw offsets and strings for symbols. The Details offset editor accepts numeric or symbol-like text, displays concrete numeric offsets as `0x...`, rejects number-like symbol names through parser validation, and keeps symbolic chains unresolved for runtime dereference until scoped symbol resolution lands. This needs human verification in the running UI.
