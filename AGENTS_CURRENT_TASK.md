...
- needs human verification: First unified target slice is in place. `ProjectItem` now stores a `ProjectItemTarget` enum with address, pointer path, symbol, plugin, and none variants; address/pointer/symbol-ref constructors populate it; core project-item symbol/location resolution prefers the target enum; and `ProjectItemsCreateRequest` no longer carries `symbol_locator_display`. Symbol Tree module-field rows now use `module_field:` node identity and selection canonicalizes module-field symbol locators to derived node selections, avoiding post-split selection collisions with catalog symbol claims. Reverified with `cargo fmt --all`, `cargo test -p squalr-engine-api project_item -- --nocapture`, `cargo test -p squalr-engine project_items -- --nocapture`, `cargo test -p squalr symbol_explorer -- --nocapture`, `cargo check -p squalr`, and `cargo check -p squalr-tui`.
- Refactor Project Explorer project items into one user-facing item concept with a unified `ProjectItemTarget`.
...
- Update Project Explorer creation UX to create normal project items with target expressions instead of asking users to choose address/pointer/symbol-ref item types.
