...
11. In progress: move Symbol Layout authoring behavior out of the GUI view.
   ...
   - Done: moved the Symbol Layout editor takeover screens into `squalr/src/views/symbol_layout_editor/symbol_layout_editor_view/take_over_views.rs`. This is a real sub-view extraction, not a helper dump: create/rename/open layout takeovers, define-field takeover, delete-layout confirmation, and delete-field confirmation now live together outside the parent view file.
...
- Current Symbol Layout takeover extraction pass moved the takeover sub-views out of `symbol_layout_editor_view.rs` into `symbol_layout_editor_view/take_over_views.rs`, dropping the parent view from roughly 5.1k lines to 4.4k lines while preserving behavior. The extracted module owns the takeover shell, action buttons, create/rename/open layout editor screen, define-field screen, and delete confirmations. Validated with `cargo fmt --all`, `cargo check -p squalr --locked`, `cargo test -p squalr symbol_layout_editor --lib --locked`, and `cargo test -p squalr struct_viewer --lib --locked`. Needs human verification in the GUI.
