...
- Linux debugger implementation status:
  ...
  - Follow-up fix: x86/x64 instruction parsing now accepts NASM memory address hints such as `[rel 2B2Dh]`, which iced-x86's NASM formatter emits for RIP-relative memory in debug trace disassembly.
  ...
  - Rebuilt and launched current WSL GUI and watch-value helper visibly for manual attach testing. At the time of update, WSL showed `./target/debug/squalr` PID 2497 and `./target/linux-tools/squalr-watch-value` PID 2503.
