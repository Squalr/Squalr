...
- Pointer offset editing now submits and refreshes through the same CSV-style chain display text, so symbol rows do not transiently show JSON like `["Timer"]`. Numeric offset rows default to hex while still allowing string/bin/dec/hex/address formats, Symbol Tree Add to Project preserves valid module symbol roots as address-chain symbols, and address-chain symbol resolution handles module-scoped symbolic segments after reload. This needs human verification in the running UI.
...
- ProjectItemAddressTarget now has shared symbol-aware runtime pointer materialization. Project Explorer preview/value edit/context-action paths use the opened project's symbol catalog instead of direct numeric-only conversion, so address items whose first offset is a module symbol such as `game.exe+Health` can build runtime queries. Validated with focused API/engine/GUI unit tests; needs human verification in the running UI.
