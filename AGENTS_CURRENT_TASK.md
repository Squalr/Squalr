...
- needs human verification: Element Scanner and Pointer Scanner result rows now render inside an explicit post-header content viewport so partially scrolled rows cannot bleed upward into result headers/title areas. Element Scanner row text and tooltip hitboxes also preserve the inherited scroll clip.
- needs human verification: Promote-to-symbol module root creation now sizes a newly ensured module from the loaded module extent even when the base virtual page is smaller, so Symbol Tree can synthesize both preceding and trailing u8[] unassigned spans around the promoted symbol.
