...
- Resolver and resolver-node edits now route through the existing Details Viewer (`StructViewerViewData`) by focusing resolver-specific virtual fields. Node kind, operator, and type-size data type use Details Viewer presentations rather than custom controls inside the Symbol Resolvers window. Needs human verification for live GUI ergonomics.
