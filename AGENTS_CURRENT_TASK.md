...
- The Symbol Resolvers window now has two modes patterned after Project Explorer. List mode shows only resolver entries; the top bar `+` creates a resolver, and each resolver row owns its edit affordance. Selecting a resolver does not implicitly begin editing. Needs human verification for live GUI ergonomics.
- Resolver create/edit now uses an explicit takeover with a Project Editor-style header, name field, cancel action, save action, and danger-background delete action for existing resolvers. Saving exits back to list mode. Needs human verification for live GUI ergonomics.
- Inside resolver create/edit, the expression is a rooted tree editor. There is no tree add menu or morph button; node kind/operator/type details route through the existing Details Viewer (`StructViewerViewData`). Changing a node to an operation creates the fixed two child slots, and changing it back removes nesting through the same Details Viewer combo path. Changing node kind refocuses Details Viewer because the selected node's editable fields change shape. Needs human verification for live GUI ergonomics.
