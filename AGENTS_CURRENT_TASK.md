...
- PR branch review notes:
  ...
  - `squalr/src/views/project_explorer/project_hierarchy/project_hierarchy_view.rs` debugger trace start resolves module-relative project item addresses through the Project Explorer helper, but the trace request model accepts only an absolute address. If module resolution times out or fails, the helper returns the unresolved offset plus the module name and the caller drops the module name, so a trace for `module+offset` can start at raw address `offset`.
  - `squalr-engine/src/command_executors/memory/write/memory_write_request_executor.rs` still performs raw module-relative writes at address `0` when module resolution fails, then reports failure. This appears pre-existing, but the PR touches the write response/error path and checked-code writes, so it is worth fixing before merge or explicitly separating as existing debt.
...
- After investigating macOS debugger parity for `Find What Reads/Writes/Accesses`:
  - Current Squalr architecture is already close to what macOS needs. The shared debugger API, `DebuggerService` trace-session aggregation, CLI aliases, and GUI Debugger Trace surface are backend-agnostic. The missing work is a macOS implementation behind `plugins/squalr-plugin-debuggers-native/src/backend`, plus small platform binding helpers.
  - The native debugger package currently uses `windows_backend.rs` for DbgEng and routes every non-Windows platform through `non_windows_backend.rs`, whose attach/control/breakpoint/register calls intentionally return "No native debugger backend is available for this platform yet." `NativeDebuggersPlugin::can_attach` is also hard-gated to `cfg!(windows)` and x86/x64, so macOS is presently disabled before session creation.
  - Existing macOS target support already opens a Mach task port via `task_for_pid`, stores it as `OpenedProcessInfo::handle`, detects `x64`/`arm64` target architecture, and reads/writes/query memory with `mach_vm_read_overwrite`, `mach_vm_write`, and `mach_vm_region_recurse`. The debugger backend can reuse that task port for instruction-byte reads instead of adding a second memory path.
  - Preferred implementation path is a native Mach backend, not an LLDB dependency, for the first Squalr-owned MVP. Apple/Mach APIs and the existing `mach2` dependency cover most primitives: `ptrace(PT_ATTACHEXC)` for debug attach semantics, `task_suspend`/`task_resume` or per-thread suspend/resume for pause/resume, `task_threads`, `thread_get_state`, `thread_set_state`, Mach exception receive/reply, and architecture debug-state flavors. `mach2` lacks some declarations such as `task_set_exception_ports`/`task_get_exception_ports`, so add a small local FFI module rather than changing public engine API.
  - The macOS backend should be added as `backend/macos_backend.rs`, with `backend/mod.rs` selecting it under `#[cfg(target_os = "macos")]`. Keep `non_windows_backend.rs` for Linux/Android until those backends exist. Update `NativeDebuggersPlugin::can_attach` to allow macOS `x64` and `arm64` when the process handle is nonzero.
  - Attach lifecycle should run on a worker thread like DbgEng. Attach should allocate a Mach exception port, save the task's existing `EXC_MASK_BREAKPOINT` exception handlers, install Squalr's exception port with `EXCEPTION_STATE_IDENTITY | MACH_EXCEPTION_CODES`, attach/stop the target with `ptrace(PT_ATTACHEXC)` or an equivalent Mach attach sequence, and report ready only after the initial stop/event context can be inspected. Detach should disable all Squalr-installed watchpoints on every thread, restore previous exception ports, resume the task, detach ptrace state if used, deallocate Mach ports, and avoid killing or leaving the target suspended.
  - Hardware data breakpoints map naturally to CPU watchpoint/debug registers, but implementation is architecture-specific. On x86_64, use `x86_DEBUG_STATE64` and debug registers `dr0`-`dr7`; support only the four hardware slots and Squalr's 1/2/4/8 byte sizes. On arm64, use `ARM_DEBUG_STATE64` watchpoint value/control registers; support the hardware slot count reported by the state shape, and map Squalr read/write/access modes onto ARM watchpoint load/store controls. All current and newly created target threads must receive the active watchpoint set.
  - Trace event handling should mirror the DbgEng behavior, not generic breakpoint UI behavior. When an `EXC_BREAKPOINT` arrives for a Squalr hardware data watchpoint, select the stopped thread, capture registers, read instruction bytes from the access instruction, emit `DebuggerTraceEvent`, then immediately resume if the breakpoint belongs to a trace session. Non-trace software/code breakpoints can remain paused later.
  - Instruction attribution is the highest-risk parity detail. Windows needed special handling because x86/x64 data breakpoints report after the access instruction, so DbgEng uses `GetNearInstruction(post_trap_ip, -1)`. macOS should implement equivalent attribution per architecture: x86_64 likely needs previous-instruction attribution after the hardware trap, while arm64 watchpoint exceptions may expose the faulting PC directly. This needs disposable-process live smokes on Intel and Apple Silicon before trusting GUI results.
  - Register snapshots need per-architecture flattening into Squalr's generic integer register model. MVP should capture IP/SP plus general-purpose registers for x86_64 and arm64 using `thread_get_state` (`x86_THREAD_STATE64` / `ARM_THREAD_STATE64`) and skip vector/FP state as DbgEng currently does for non-integer values. Register writes can initially support integer registers only, matching the generic API and DbgEng precedent.
  - Software breakpoints are not required for `Find What Reads/Writes/Accesses` parity because trace sessions use `DebuggerBreakpointKind::HardwareData`. If implemented later, prefer the existing patch-service-backed software breakpoint flow instead of teaching the macOS backend to patch `int3`/`brk` directly.
  - Thread management is a real MVP requirement, not polish. A watchpoint set only on the currently stopped thread will miss writes from other threads. The backend needs a tracked thread list refresh at attach and after thread-create exceptions or periodic event handling. New threads should receive active watchpoints before the target is resumed.
  - Expected security model remains the README macOS model: SIP/debug restrictions must already be handled by the user; assume SIP disabled and elevated execution for development. Even then, protected or hardened targets may reject `task_for_pid`/debug attach and should produce normal `DebuggerPluginError` messages.
  - Suggested validation ladder:
    - Compile-only: `cargo check -p squalr-plugin-debuggers-native --locked` on macOS x64 and arm64.
    - Unit/fake tests: breakpoint descriptor bookkeeping, watchpoint slot allocation/size validation, register-name mapping, exception-message classification, and trace-event aggregation through existing `squalr-engine-session` tests.
    - Live plugin smoke: extend `plugins/squalr-plugin-debuggers-native/examples/native_debugger_smoke.rs` to run on macOS by spawning a disposable child with a known `AtomicU64`, attach, set a hardware write watchpoint, resume, receive one trace event, stop trace/remove watchpoint, and detach.
    - Live service/command smokes: reuse `squalr-engine-session/examples/debugger_service_native_smoke.rs` and `squalr-engine/examples/debugger_command_native_smoke.rs` after backend support exists.
    - Human verification: CLI `find-what-writes`, TUI/GUI attach-confirmation flow, Debugger Trace row updates, Stop Trace, target-survives-detach, and attribution against a known write instruction on both Intel and Apple Silicon if available.
  - Official/reference material checked during investigation:
    - Apple `thread_get_state` / `thread_set_state` docs.
    - Apple `task_set_exception_ports` / `task_get_exception_ports` docs.
    - Apple `ptrace(2)` man page for `PT_ATTACHEXC`.
