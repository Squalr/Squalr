...
11. In progress: move Symbol Layout authoring behavior out of the GUI view.
   ...
   - `SymbolLayoutEditorViewData` still owns GUI draft construction/descriptor validation, but no longer owns catalog upsert/delete/retarget mutation.
   - The remaining whole-catalog `set-catalog` use in Symbol Layout editor is rollback-only for the current side-effectful union-variant edit flow; removing that cleanly needs an explicit editor transaction/undo command rather than ad hoc catalog replacement.
