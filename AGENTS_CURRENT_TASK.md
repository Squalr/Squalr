...
- Continue target/scanning boundary cleanup after adding the public element scanner facade.
...
- Current scanning API cleanup added `ElementScanner` / `ElementScanReport` / `ScanControl` public reexports, added `Snapshot::from_regions` and `SnapshotRegion::from_bytes` helpers for caller-owned bytes, routed `ElementScanExecutor` through `ElementScanner::scan_snapshot_with_region_refresh`, and switched engine call sites to root `squalr_engine_scanning` reexports instead of task module paths. Public scanner APIs can now scan precollected snapshots without `OpenedProcessInfo`, while the legacy executor remains as the session/target adapter. Validated with `cargo fmt --all`, `cargo test -p squalr-engine-scanning --locked`, `cargo test -p squalr-engine-api --locked`, `cargo check -p squalr-engine --locked`, `cargo test -p squalr-engine scan --lib --locked`, `cargo check -p squalr --locked`, `cargo check -p squalr-cli --locked`, `cargo check -p squalr-tui --locked`, and search for external `squalr_engine_scanning::scanners` imports. Needs human verification for GUI/CLI/TUI scan workflows and external consumers adopting the new facade.
