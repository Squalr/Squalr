...
- needs human verification: Project Explorer details now exposes a target-kind selector for normal items and target-specific editable fields for address, pointer, symbol, and a minimal plugin target shape. Target edits persist through the existing details edit path and project item refresh now compares target content. Plugin payload display is read-only until the target plugin/editor registration model exists.
