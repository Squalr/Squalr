...
- 2026-03-15: Installer action rows now use fixed-height left-aligned button rows, and the duplicate outer log wrapper was removed so the log view sizes its scroll region from the actual remaining height. Need human verification in the installer UI.
