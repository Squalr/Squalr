...
- Project Explorer Details pointer-offset preview is a readonly string DataValueBox showing CSV-style offsets without the old commit-slot padding. Its edit icon opens a Details/Struct Viewer takeover repeater with DataValueBox offset rows and add/delete controls, then persists through the address-target edit path. This needs human verification in the running UI.
