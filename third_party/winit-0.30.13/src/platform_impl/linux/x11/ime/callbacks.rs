...
unsafe fn replace_im(inner: *mut ImeInner) -> Result<(), ReplaceImError> {
    ...
    let (new_im, is_fallback) = {
        ...
        (
            new_im
                .ok()
                .ok_or_else(|| ReplaceImError::MethodOpenFailed(Box::new(unsafe { (*inner).potential_input_methods.clone() })))?,
            is_fallback,
        )
    };
    ...
    for (window, old_context) in unsafe { (*inner).contexts.iter() } {
        ...
        let new_context = {
            let result = unsafe { ImeContext::new(xconn, &new_im, *window, spot, (*inner).event_sender.clone(), is_allowed) };
            if result.is_err() {
                let _ = unsafe { close_im(xconn, new_im.im) };
            }
