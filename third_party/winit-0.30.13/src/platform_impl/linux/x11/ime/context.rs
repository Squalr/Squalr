...
extern "C" fn preedit_draw_callback(
    ...
    client_data
        .event_sender
        .send((client_data.window, ImeEvent::Update(client_data.text.iter().collect(), cursor_byte_pos)))
...
impl PreeditCallbacks {
    pub fn new(client_data: ffi::XPointer) -> PreeditCallbacks {
        let start_callback = create_xim_callback(client_data, unsafe {
            mem::transmute::<usize, unsafe extern "C" fn(ffi::XIM, ffi::XPointer, ffi::XPointer)>(preedit_start_callback as *const () as usize)
        });
...
impl ImeContext {
    pub(crate) unsafe fn new(
        ...
        let ic = match style as _ {
            Style::Preedit(style) => unsafe { ImeContext::create_preedit_ic(xconn, im.im, style, window, client_data as ffi::XPointer) },
            Style::Nothing(style) => unsafe { ImeContext::create_nothing_ic(xconn, im.im, style, window) },
    ...
    pub(crate) fn set_spot(
        ...
        unsafe {
            let preedit_attr = util::memory::XSmartPointer::new(
                xconn,
                (xconn.xlib.XVaCreateNestedList)(0, ffi::XNSpotLocation_0.as_ptr(), &self.ic_spot, ptr::null_mut::<()>()),
            )
            .expect("XVaCreateNestedList returned NULL");
