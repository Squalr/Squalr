...
use tracing::warn;
use xkbcommon_dl::{self as xkb, xkb_compose_status, xkb_context, xkb_context_flags, xkbcommon_compose_handle, xkbcommon_handle, XkbCommon, XkbCommonCompose};
#[cfg(x11_platform)]
...
impl KeyContext<'_> {
    pub fn process_key_event(
        ...
        let mut event = KeyEventResults::new(self, keycode, !repeat && state == ElementState::Pressed);
    ...
    fn keysym_to_utf8_raw(
        ...
        loop {
            let bytes_written = unsafe { (XKBH.xkb_keysym_to_utf8)(keysym, self.scratch_buffer.as_mut_ptr().cast(), self.scratch_buffer.capacity()) };
            if bytes_written == 0 {
                return None;
...
impl<'a, 'b> KeyEventResults<'a, 'b> {
    ...
    fn keysym_to_key(
        &self,
        keysym: u32,
    ) -> Result<(Key, KeyLocation), (Key, KeyLocation)> {
    ...
    fn composed_text(&mut self) -> Result<Option<SmolStr>, ()> {
        match self.compose {
            ComposeStatus::Accepted(status) => match status {
                ...
                xkb_compose_status::XKB_COMPOSE_COMPOSING | xkb_compose_status::XKB_COMPOSE_CANCELLED => Ok(None),
...
fn byte_slice_to_smol_str(bytes: &[u8]) -> Option<SmolStr> {
    std::str::from_utf8(bytes)
        .map(SmolStr::new)
        .map_err(|e| tracing::warn!("UTF-8 received from libxkbcommon ({:?}) was invalid: {e}", bytes))
        .ok()
}
