...
use xkbcommon_dl::{
    xkb_compose_compile_flags, xkb_compose_feed_result, xkb_compose_state, xkb_compose_state_flags, xkb_compose_status, xkb_compose_table, xkb_keysym_t,
};
...
impl XkbComposeTable {
    pub fn new(context: &XkbContext) -> Option<Self> {
        ...
        let table =
            unsafe { (XKBCH.xkb_compose_table_new_from_locale)(context.as_ptr(), locale.as_ptr(), xkb_compose_compile_flags::XKB_COMPOSE_COMPILE_NO_FLAGS) };
    ...
    pub fn new_state(&self) -> Option<XkbComposeState> {
        let state = unsafe { (XKBCH.xkb_compose_state_new)(self.table.as_ptr(), xkb_compose_state_flags::XKB_COMPOSE_STATE_NO_FLAGS) };
...
impl XkbComposeState {
    ...
    pub fn feed(
        ...
        match feed_result {
            ...
            xkb_compose_feed_result::XKB_COMPOSE_FEED_ACCEPTED => ComposeStatus::Accepted(self.status()),
        }
    }
