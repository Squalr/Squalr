...
use web_sys::{Document, HtmlCanvasElement, Navigator, PageTransitionEvent, VisibilityState, Window};
...
pub fn on_page_transition(
    ...
    let show_listener = event_handle::EventListenerHandle::new(window.clone(), "pageshow", show_closure);
...
pub fn set_canvas_position(
    ...
    if document.contains(Some(raw)) && style.get("display") != "none" {
        position.x -= style_size_property(style, "margin-left") + style_size_property(style, "border-left-width") + style_size_property(style, "padding-left");
        position.y -= style_size_property(style, "margin-top") + style_size_property(style, "border-top-width") + style_size_property(style, "padding-top");
    }
