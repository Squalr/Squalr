...
impl PointerHandler {
    ...
    {
        self.on_cursor_leave = Some(canvas_common.add_event("pointerout", move |event: PointerEvent| {
    ...
    pub fn on_cursor_enter<F>(
        ...
        F: 'static + FnMut(ModifiersState, Option<i32>),
    {
        self.on_cursor_enter = Some(canvas_common.add_event("pointerover", move |event: PointerEvent| {
            let modifiers = event::mouse_modifiers(&event);
        ...
        T: 'static + FnMut(ModifiersState, i32, PhysicalPosition<f64>, Force),
    {
        ...
        self.on_pointer_release = Some(canvas_common.add_event("pointerup", move |event: PointerEvent| {
            let modifiers = event::mouse_modifiers(&event);
        ...
        T: 'static + FnMut(ModifiersState, i32, PhysicalPosition<f64>, Force),
    {
        ...
        self.on_pointer_press = Some(canvas_common.add_event("pointerdown", move |event: PointerEvent| {
        ...
        B: 'static + FnMut(ModifiersState, i32, PhysicalPosition<f64>, ButtonsState, MouseButton),
    {
        let window = canvas_common.window.clone();
        let canvas = canvas_common.raw().clone();
        self.on_cursor_move = Some(canvas_common.add_event("pointermove", move |event: PointerEvent| {
            let modifiers = event::mouse_modifiers(&event);
            ...
            match event.pointer_type().as_str() {
                "touch" => touch_handler(
                    ...
                    &mut event::pointer_move_event(event)
                        .map(|event| (event::mouse_position(&event).to_physical(scale), Force::Normalized(event.pressure() as f64))),
                ),
                _ => mouse_handler(
                    modifiers,
                    id,
                    &mut event::pointer_move_event(event).map(|event| event::mouse_position(&event).to_physical(scale)),
                ),
            };
        }));
    }
    ...
    {
        let window = canvas_common.window.clone();
        self.on_touch_cancel = Some(canvas_common.add_event("pointercancel", move |event: PointerEvent| {
