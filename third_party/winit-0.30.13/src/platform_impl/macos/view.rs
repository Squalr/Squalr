...
use objc2_app_kit::{
    NSApplication, NSCursor, NSEvent, NSEventPhase, NSResponder, NSTextInputClient, NSTrackingRectTag, NSView, NSViewFrameDidChangeNotification,
};
use objc2_foundation::{
    MainThreadMarker, NSArray, NSAttributedString, NSAttributedStringKey, NSCopying, NSMutableAttributedString, NSNotFound, NSNotificationCenter, NSObject,
    NSObjectProtocol, NSPoint, NSRange, NSRect, NSSize, NSString, NSUInteger,
};

use super::app_state::ApplicationDelegate;
use super::cursor::{default_cursor, invisible_cursor};
use super::event::{code_to_key, code_to_location, create_key_event, event_mods, lalt_pressed, ralt_pressed, scancode_to_physicalkey, KeyEventExtra};
use super::window::WinitWindow;
use super::DEVICE_ID;
use crate::dpi::{LogicalPosition, LogicalSize};
use crate::event::{DeviceEvent, ElementState, Ime, KeyEvent, Modifiers, MouseButton, MouseScrollDelta, TouchPhase, WindowEvent};
...
impl WinitView {
    pub(super) fn new(
        ...
        unsafe { notification_center.addObserver_selector_name_object(&this, sel!(frameDidChange:), Some(NSViewFrameDidChangeNotification), Some(&this)) }
    ...
    pub(super) fn set_cursor_icon(
        &self,
        icon: Retained<NSCursor>,
    ) {
    ...
    pub(super) fn set_cursor_visible(
        &self,
        visible: bool,
    ) -> bool {
        let mut cursor_state = self.ivars().cursor_state.borrow_mut();
    ...
    pub(super) fn set_option_as_alt(
        &self,
        value: OptionAsAlt,
    ) {
    ...
    fn mouse_motion(
        ...
        if view_point.x.is_sign_negative() || view_point.y.is_sign_negative() || view_point.x > frame.size.width || view_point.y > frame.size.height {
