...
use windows_sys::Win32::UI::Input::Ime::{
    ImmAssociateContextEx, ImmGetCompositionStringW, ImmGetContext, ImmReleaseContext, ImmSetCandidateWindow, ImmSetCompositionWindow, ATTR_TARGET_CONVERTED,
    ATTR_TARGET_NOTCONVERTED, CANDIDATEFORM, CFS_EXCLUDE, CFS_POINT, COMPOSITIONFORM, GCS_COMPATTR, GCS_COMPSTR, GCS_CURSORPOS, GCS_RESULTSTR, IACE_CHILDREN,
    IACE_DEFAULT,
};
...
impl ImeContext {
    ...
    unsafe fn get_composition_cursor(
        &self,
        text: &str,
    ) -> Option<usize> {
    ...
    unsafe fn get_composition_data(
        &self,
        gcs_mode: u32,
    ) -> Option<Vec<u8>> {
        ...
        let size = unsafe { ImmGetCompositionStringW(self.himc, gcs_mode, buf.as_mut_ptr() as *mut c_void, size as _) };
