...
pub type XlibErrorHook = Box<dyn Fn(*mut std::ffi::c_void, *mut std::ffi::c_void) -> bool + Send + Sync>;
...
pub trait WindowAttributesExtX11 {
    ...
    fn with_x11_window_type(
        self,
        x11_window_type: Vec<WindowType>,
    ) -> Self;
    ...
    /// # Ok(()) }
    /// ```
    fn with_embed_parent_window(
        self,
        parent_window_id: XWindow,
    ) -> Self;
}
...
impl WindowAttributesExtX11 for WindowAttributes {
    ...
    fn with_name(
        ...
        self.platform_specific.name = Some(crate::platform_impl::ApplicationName::new(general.into(), instance.into()));
        self
    }
