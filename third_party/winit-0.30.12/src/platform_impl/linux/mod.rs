...
use crate::event_loop::{ActiveEventLoop as RootELW, AsyncRequestSerial, ControlFlow, DeviceEvents, EventLoopClosed};
use crate::icon::Icon;
...
use crate::window::{
    ActivationToken, Cursor, CursorGrabMode, CustomCursor, CustomCursorSource, ImePurpose, ResizeDirection, Theme, UserAttentionType, WindowAttributes,
...
impl Window {
    ...
    pub(crate) fn new(
        ...
        match *window_target {
            #[cfg(wayland_platform)]
            ActiveEventLoop::Wayland(ref window_target) => wayland::Window::new(window_target, attribs).map(Window::Wayland),
            #[cfg(x11_platform)]
            ActiveEventLoop::X(ref window_target) => x11::Window::new(window_target, attribs).map(Window::X),
        }
    }
...
unsafe extern "C" fn x_error_callback(
    ...
    if let Ok(ref xconn) = *xconn_lock {
        ...
        unsafe { (xconn.xlib.XGetErrorText)(display, (*event).error_code as c_int, buf.as_mut_ptr() as *mut c_char, buf.len() as c_int) };
        let description = unsafe { CStr::from_ptr(buf.as_ptr() as *const c_char) }.to_string_lossy();
...
impl<T: 'static> EventLoop<T> {
    pub(crate) fn new(attributes: &PlatformSpecificEventLoopAttributes) -> Result<Self, EventLoopError> {
    ...
    fn new_x11_any_thread() -> Result<EventLoop<T>, EventLoopError> {
        let xconn = match X11_BACKEND.lock().unwrap_or_else(|e| e.into_inner()).as_ref() {
            ...
            Err(err) => return Err(EventLoopError::Os(os_error!(OsError::XNotSupported(err.clone())))),
        };
...
impl ActiveEventLoop {
    ...
    pub fn available_monitors(&self) -> VecDeque<MonitorHandle> {
        match *self {
            #[cfg(wayland_platform)]
            ActiveEventLoop::Wayland(ref evlp) => evlp.available_monitors().map(MonitorHandle::Wayland).collect(),
            #[cfg(x11_platform)]
            ActiveEventLoop::X(ref evlp) => evlp.available_monitors().map(MonitorHandle::X).collect(),
        }
    }

    #[inline]
    pub fn primary_monitor(&self) -> Option<MonitorHandle> {
        Some(x11_or_wayland!(match self; ActiveEventLoop(evlp) => evlp.primary_monitor()?; as MonitorHandle))
    }
...
impl OwnedDisplayHandle {
    ...
    pub fn raw_display_handle_rwh_06(&self) -> Result<rwh_06::RawDisplayHandle, rwh_06::HandleError> {
        ...
        match self {
            #[cfg(x11_platform)]
            Self::X(xconn) => Ok(rwh_06::XlibDisplayHandle::new(NonNull::new(xconn.display.cast()), xconn.default_screen_index() as _).into()),

            #[cfg(wayland_platform)]
            Self::Wayland(conn) => {
                use sctk::reexports::client::Proxy;

                Ok(rwh_06::WaylandDisplayHandle::new(NonNull::new(conn.display().id().as_ptr().cast()).unwrap()).into())
            }
        }
    }
}
