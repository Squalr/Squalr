...
impl ModifierKeymap {
    ...
    pub fn reload_from_x_connection(
        &mut self,
        xconn: &super::XConnection,
    ) {
        unsafe {
    ...
    fn reset_from_x_keymap(
        &mut self,
        keymap: &XModifierKeymap,
    ) {
        let keys_per_mod = keymap.max_keypermod as usize;

        let keys = unsafe { slice::from_raw_parts(keymap.modifiermap as *const _, keys_per_mod * NUM_MODS) };
