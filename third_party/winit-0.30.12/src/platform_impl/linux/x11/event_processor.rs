...
use x11_dl::xinput2::{self, XIDeviceEvent, XIEnterEvent, XIFocusInEvent, XIFocusOutEvent, XIHierarchyEvent, XILeaveEvent, XIModifierState, XIRawEvent};
use x11_dl::xlib::{
    self, Display as XDisplay, Window as XWindow, XAnyEvent, XClientMessageEvent, XConfigureEvent, XDestroyWindowEvent, XEvent, XExposeEvent, XKeyEvent,
    XMapEvent, XPropertyEvent, XReparentEvent, XSelectionEvent, XVisibilityEvent, XkbAnyEvent, XkbStateRec,
};
...
use crate::event::{DeviceEvent, ElementState, Event, Ime, InnerSizeWriter, MouseButton, MouseScrollDelta, RawKeyEvent, Touch, TouchPhase, WindowEvent};
...
use crate::platform_impl::x11::{
    mkdid, mkwid, util, CookieResultExt, Device, DeviceId, DeviceInfo, Dnd, DndState, ImeReceiver, ScrollOrientation, UnownedWindow, WindowId,
};
...
impl EventProcessor {
    pub fn process_event<T: 'static, F>(
        &mut self,
        xev: &mut XEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        while let Ok((window, event)) = self.ime_event_receiver.try_recv() {
            ...
            let event = match event {
                ...
                ImeEvent::Update(text, position) if self.is_composing => WindowEvent::Ime(Ime::Preedit(text, Some((position, position)))),
                ImeEvent::End => {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        match event_type {
            ...
            xlib::GenericEvent => {
                let wt = Self::window_target(&self.target);
                let xev: GenericEventCookie = match GenericEventCookie::from_event(wt.xconn.clone(), *xev) {
                ...
                match evtype {
                    ty @ xinput2::XI_ButtonPress | ty @ xinput2::XI_ButtonRelease => {
                        ...
                        self.update_mods_from_xinput2_event(&xev.mods, &xev.group, false, &mut callback);
    ...
    pub unsafe fn poll_one_event(
        ...
        let result = unsafe { (window_target.xconn.xlib.XCheckIfEvent)(window_target.xconn.display, event_ptr, Some(predicate), std::ptr::null_mut()) };
    ...
    fn selection_notify<T: 'static, F>(
        &mut self,
        xev: &XSelectionEvent,
        mut callback: F,
    ) where
    ...
    fn configure_notify<T: 'static, F>(
        &self,
        xev: &XConfigureEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if is_synthetic {
            ...
            if last_scale_factor != new_scale_factor {
                let (new_width, new_height) = window.adjust_for_dpi(last_scale_factor, new_scale_factor, width, height, &shared_state_lock);
    ...
    fn destroy_notify<T: 'static, F>(
        &self,
        xev: &XDestroyWindowEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if atom == xproto::Atom::from(xproto::AtomEnum::RESOURCE_MANAGER) || atom == atoms[_XSETTINGS_SETTINGS] {
            self.process_dpi_change(&mut callback);
        }
    }

    fn visibility_notify<T: 'static, F>(
        &self,
        xev: &XVisibilityEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        let replay = if let Some(position) = self
            ...
            .iter()
            .rev()
            .position(|&s| s == xev.keycode as u8)
        {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        let mask = unsafe { slice::from_raw_parts(event.valuators.mask, event.valuators.mask_len as usize) };
        let mut devices = self.devices.borrow_mut();
        ...
        for i in 0..event.valuators.mask_len * 8 {
                ...
                .find(|&&mut (axis, _)| axis == i as _)
            {
                ...
                let delta = match info.orientation {
                    ScrollOrientation::Horizontal => MouseScrollDelta::LineDelta(-delta as f32, 0.0),
    ...
    fn xinput2_mouse_enter<T: 'static, F>(
        &self,
        event: &XIEnterEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if let Some(all_info) = DeviceInfo::get(&wt.xconn, super::ALL_DEVICES.into()) {
            ...
            for device_info in all_info.iter() {
                ...
                if device_info.deviceid == event.sourceid || device_info.attachment == event.sourceid {
    ...
    fn xinput2_mouse_left<T: 'static, F>(
        &self,
        event: &XILeaveEvent,
        mut callback: F,
    ) where
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        Self::handle_pressed_keys(&self.target, window_id, ElementState::Pressed, &mut self.xkb_context, &mut callback);
    ...
    fn xinput2_unfocused<T: 'static, F>(
        &mut self,
        xev: &XIFocusOutEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        if self.active_window.take() == Some(window) {
            ...
            Self::handle_pressed_keys(&self.target, window_id, ElementState::Released, &mut self.xkb_context, &mut callback);
    ...
    fn xinput2_raw_mouse_motion<T: 'static, F>(
        &self,
        xev: &XIRawEvent,
        mut callback: F,
    ) where
    ...
    fn xinput2_hierarchy_changed<T: 'static, F>(
        &mut self,
        xev: &XIHierarchyEvent,
        mut callback: F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        match xev.xkb_type {
            xlib::XkbNewKeyboardNotify => {
                ...
                if xev.device == self.xkb_context.core_keyboard_id && (keycodes_changed || geometry_changed) {
                    let xcb = wt.xconn.xcb_connection().get_raw_xcb_connection();
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        unsafe {
            ...
            if (wt.xconn.xlib.XkbGetState)(wt.xconn.display, XkbId::USE_CORE_KBD.into(), &mut state) == xlib::True {
        ...
        F: FnMut(&RootAEL, Event<T>),
    {
        ...
        xkb_state.update_modifiers(depressed, latched, locked, 0, 0, Self::core_keyboard_group(state));
    ...
    fn process_dpi_change<T: 'static, F>(
        &self,
        callback: &mut F,
    ) where
        F: FnMut(&RootAEL, Event<T>),
