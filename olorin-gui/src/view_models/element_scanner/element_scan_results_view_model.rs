use crate::ElementScanResultsViewModelBindings;
...
pub struct ElementScanResultsViewModel {
...
impl ElementScanResultsViewModel {
    ...
    fn on_dependencies_resolved(
        ...
        let scan_results_collection = create_view_model_collection!(
            ...
            ElementScanResultsViewModelBindings -> { set_scan_results, get_scan_results },
            ScanResultConverter -> [],
        );
        ...
        let view_model = Arc::new(ElementScanResultsViewModel {
...

        {
            ...
            create_view_bindings!(view_binding, {
                ElementScanResultsViewModelBindings => {
        ...
        dependency_container.register::<ElementScanResultsViewModel>(view_model);
    }
    ...
    fn poll_scan_results(view_model: Arc<ElementScanResultsViewModel>) {
    ...
    fn load_current_page_index(view_model: &Arc<ElementScanResultsViewModel>) -> u64 {
    ...
    fn query_scan_results(
        view_model: Arc<ElementScanResultsViewModel>,
        play_sound: bool,
    ) {
        let engine_execution_context = &view_model.engine_execution_context;
        ...
        scan_results_query_request.send(engine_execution_context, move |scan_results_query_response| {
            ...
            view_binding.execute_on_ui_thread(move |main_window_view, _| {
                ...
                let scan_results_bindings = main_window_view.global::<ElementScanResultsViewModelBindings>();
    ...
    fn refresh_scan_results(view_model: Arc<ElementScanResultsViewModel>) {
    ...
    fn on_navigate_first_page(view_model: Arc<ElementScanResultsViewModel>) {
        let new_page_index = 0;

        Self::set_page_index(view_model, new_page_index);
    }

    fn on_navigate_last_page(view_model: Arc<ElementScanResultsViewModel>) {
        let cached_last_page_index = &view_model.cached_last_page_index;
    ...
    fn on_navigate_previous_page(view_model: Arc<ElementScanResultsViewModel>) {
        let new_page_index = Self::load_current_page_index(&view_model).saturating_sub(1);

        Self::set_page_index(view_model, new_page_index);
    }

    fn on_navigate_next_page(view_model: Arc<ElementScanResultsViewModel>) {
    ...
    fn on_add_scan_results_to_project(view_model: Arc<ElementScanResultsViewModel>) {
    ...
    fn on_delete_selected_scan_results(view_model: Arc<ElementScanResultsViewModel>) {
        let scan_result_refs = Self::collect_selected_scan_result_refs(&view_model);
    ...
    fn on_toggle_selected_scan_results_frozen(view_model: Arc<ElementScanResultsViewModel>) {
        /*
    ...
    fn collect_selected_scan_result_refs(view_model: &Arc<ElementScanResultsViewModel>) -> Vec<ScanResultRef> {
    ...
    fn collect_selected_scan_result_bases(view_model: &Arc<ElementScanResultsViewModel>) -> Vec<ScanResultBase> {
        Self::collect_selected_scan_results(view_model)
            .into_iter()
    ...
    fn collect_selected_scan_results(view_model: &Arc<ElementScanResultsViewModel>) -> Vec<ScanResult> {
        let base_scan_results_collection = &view_model.base_scan_results_collection;
