use squalr_engine_api::{
    ...
    structures::debugger::{DebuggerTraceInstructionRecord, DebuggerTraceSessionDescriptor},
};
...
impl DebuggerTraceViewData {
    ...
    pub fn clear(&self) {
        match self.inner.write() {
            Ok(mut state) => {
                *state = DebuggerTraceViewState::default();
            }
            Err(error) => {
                log::error!("Failed to clear debugger trace view data: {}", error);
            }
        }
    }
    ...
    pub fn set_instruction_selected(
        &self,
        ...
        match self.inner.write() {
            Ok(mut state) => {
                let already_selected = state.selected_instruction_keys.contains(&instruction_key);

                if is_selected && !already_selected {
                    state.selected_instruction_keys.push(instruction_key);
                } else if !is_selected && already_selected {
                    state
                        .selected_instruction_keys
                        .retain(|candidate_instruction_key| candidate_instruction_key != &instruction_key);
                }
            }
            Err(error) => {
                log::error!("Failed to select debugger trace instruction: {}", error);
            }
        }
    }
    ...
    pub fn clear_selection(&self) {
        match self.inner.write() {
            Ok(mut state) => state.selected_instruction_keys.clear(),
            Err(error) => {
                log::error!("Failed to clear debugger trace instruction selection: {}", error);
            }
        }
    }
}
