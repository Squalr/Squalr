...
type CallbackWrapper<Arguments, Result = ()> = Cell<Option<Box<dyn FnMut(&Arguments, &mut Result)>>>;
...
impl<Arguments: ?Sized, Result: Default> Callback<Arguments, Result> {
    pub fn on(
        &self,
        mut f: impl FnMut(&Arguments) -> Result + 'static,
    ) {
        self.callback
            .set(Some(Box::new(move |a: &Arguments, r: &mut Result| *r = f(a))));
    }
