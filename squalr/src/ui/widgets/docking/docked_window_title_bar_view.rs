...
impl eframe::egui::Widget for DockedWindowTitleBarView {
    fn ui(
        ...
        let (rect, response) = ui.allocate_exact_size(vec2(ui.available_width(), self.height), Sense::click_and_drag());
        ...
        child_ui.label(
            RichText::new(self.identifier.clone()) // Replace with call to get_tab_text if needed
                .color(self.theme.foreground)
                .font(self.theme.font_library.font_noto_sans.font_window_title.clone()),
        );

        child_ui.add_space(child_ui.available_width());
        ...
        let drag_rect = Rect::from_min_max(rect.min, pos2(rect.max.x - 36.0, rect.max.y));
        let drag = ui.interact(drag_rect, Id::new(format!("dock_titlebar_{}", self.identifier)), Sense::click_and_drag());
