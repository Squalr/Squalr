...
- The GUI uses a workspace patch for `winit 0.30.13` under `third_party/winit-0.30.13`. The patch keeps upstream Android key-event text handling and also bridges GameActivity `TextEvent` deltas into synthetic winit key-text events so `egui-winit` can feed focused text widgets through its normal `Event::Text` path.
