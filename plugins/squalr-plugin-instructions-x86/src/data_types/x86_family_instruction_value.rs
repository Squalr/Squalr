...
pub fn x86_family_deanonymize_instruction_value(
    data_type_id: &str,
    ...
    let value_bytes = match anonymous_value_string.get_anonymous_value_string_format() {
        ...
        AnonymousValueStringFormat::Hexadecimal => x86_family_parse_hex_byte_string(anonymous_value_string.get_anonymous_value_string())?,
        unsupported_format => {
...
pub fn x86_family_anonymize_instruction_bytes(
    instruction_set: &dyn InstructionSet,
    data_type_id: &str,
    value_bytes: &[u8],
    ...
    let anonymous_value_string = match anonymous_value_string_format {
        AnonymousValueStringFormat::String => instruction_set
            ...
            .map_err(DataTypeError::ParseError)?,
        AnonymousValueStringFormat::Hexadecimal => x86_family_format_hex_byte_string(value_bytes),
        unsupported_format => {
            return Err(DataTypeError::ParseError(format!(
                "Unsupported {} format '{}' for ISA data type '{}'.",
                instruction_set.get_display_name(),
...
fn x86_family_parse_hex_byte_string(hex_bytes: &str) -> Result<Vec<u8>, DataTypeError> {
    ...
    if explicit_tokens.len() > 1 {
        return explicit_tokens
            .into_iter()
            .map(x86_family_parse_hex_byte_token)
            .collect();
    }
    ...
    collapsed_hex_bytes
        ...
        .map(|hex_pair| {
            ...
            x86_family_parse_hex_byte_token(token)
        })
        .collect()
}

fn x86_family_parse_hex_byte_token(token: &str) -> Result<u8, DataTypeError> {
...
fn x86_family_format_hex_byte_string(value_bytes: &[u8]) -> String {
    value_bytes
        .iter()
