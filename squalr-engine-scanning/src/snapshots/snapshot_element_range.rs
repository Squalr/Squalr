...
pub struct SnapshotElementRange<'a> {
    pub parent_region: &'a SnapshotRegion,
    pub region_offset: usize,
    pub range: usize,
}

impl<'a> SnapshotElementRange<'a> {
    pub fn new(parent_region: &'a SnapshotRegion) -> Self {
        Self::with_offset_and_range(parent_region, 0, parent_region.get_region_size() as usize)
    }

    pub fn with_offset_and_range(parent_region: &'a SnapshotRegion, region_offset: usize, range: usize) -> Self {
        Self {
            parent_region,
            region_offset,
            range,
    ...
    pub fn get_base_element_address(&self) -> u64 {
        self.parent_region.get_base_address() + self.region_offset as u64
    }
    ...
    pub fn get_byte_count(&self, data_type_size: usize) -> usize {
        ...
        let available_spill_over_bytes = (self.parent_region.get_base_address() + self.range as u64) - self.get_end_element_address();
    ...
    pub fn resize_for_safe_reading(&mut self, data_type_size: usize) {
        let parent_region_size = self.parent_region.get_region_size() as usize;
