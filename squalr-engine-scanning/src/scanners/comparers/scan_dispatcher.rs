...
pub fn dispatch_scan(element_range: &SnapshotElementRange, constraints: &ScanConstraints) -> Arc<dyn SnapshotElementRangeScanner + Send + Sync> {
    if element_range.get_range() == constraints.get_byte_alignment() as i32 {
        // Single element scanner
        return SnapshotElementRangeScannerScalarSingleElement::get_instance();
    } else if vectors::has_vector_support() && element_range.parent_region.region_size >= vectors::get_hardware_vector_size() {
