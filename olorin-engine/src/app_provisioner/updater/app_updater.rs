...
use olorin_engine_common::file_system::file_system_utils::FileSystemUtils;
use std::{
    path::{Path, PathBuf},
    str::FromStr,
};
        ...
        VersionCheckerTask::run(move |status| {
            if let VersionCheckerStatus::LatestVersionFound(latest_version_info) = status {
                ...
                if latest_version <= current_version {
                    log::info!("Olorin is up to date, no updates available.");
                    return;
                }
                ...
                let maybe_zip_asset = latest_version_info.assets.as_ref().and_then(|assets| {
                    assets
                        .iter()
                        .find(|asset| asset.name.eq_ignore_ascii_case("olorin.zip"))
                });
                let Some(zip_asset) = maybe_zip_asset else {
                    log::error!("Could not find olorin.zip in release assets, update failed.");
                    return;
                };
                ...
                let target_exe = install_dir.join("olorin.exe");
                ...
                let tmp_dir = match tempfile::Builder::new().prefix("olorin").tempdir() {
                    Ok(dir) => dir,
                    Err(error) => {
                ...
                let new_exe = tmp_extract_dir.join("olorin.exe");
