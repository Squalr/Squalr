            ...
            for result_index in index_of_first_page_entry..index_of_last_page_entry {
                ...
                if let Some(opened_process_info) = engine_privileged_state.get_opened_process() {
                    if let Some(mut data_value) = scan_result_base.get_data_type().get_default_value() {
                        if MemoryReader::get_instance().read(&opened_process_info, scan_result_base.get_address(), &mut data_value) {
                ...
                if let Some((found_module_name, address)) = MemoryQueryer::get_instance().address_to_module(scan_result_base.get_address(), &modules) {
                ...
                let is_frozen = snapshot
                    ...
                    .is_address_frozen(scan_result_base.get_address());
