...
fn run_smoke_against_child(child_process: &mut std::process::Child) -> Result<(), Box<dyn Error>> {
    ...
    let opened_process_info = OpenedProcessInfo::new(
        target_process_id,
        String::from("debugger_command_native_smoke_child"),
    ...
    if !plugin_registry.is_plugin_enabled("builtin.debuggers.native") && !plugin_registry.set_plugin_enabled("builtin.debuggers.native", true) {
        return Err(String::from("Failed to enable builtin.debuggers.native for command smoke validation.").into());
    }
    ...
    let attach_response = DebuggerAttachRequest {
        plugin_id: Some(String::from("builtin.debuggers.native")),
    }
...
impl EngineApiPrivilegedBindings for CapturingEngineBindings {
    ...
    fn dispatch_internal_command(
        ...
        Err(EngineBindingError::unavailable(
            "dispatching internal commands in the native debugger command smoke example",
        ))
    }
...
impl ProcessQueryProvider for NoOpProcessQueryProvider {
    ...
    fn open_process(
        ...
        Err(ProcessQueryError::internal(
            "open_process",
            "not used by the native debugger command smoke example",
        ))
    }
...
#[cfg(not(windows))]
fn main() {
    println!("The native debugger command smoke example is only available on Windows right now.");
}
